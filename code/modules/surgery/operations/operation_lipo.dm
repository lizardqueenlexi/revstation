...
/datum/surgery_operation/limb/lipoplasty/state_check(obj/item/bodypart/limb)
	if(limb.body_zone != BODY_ZONE_CHEST)
		return FALSE
	if(/*!HAS_TRAIT_FROM(limb.owner, TRAIT_FAT, OBESITY) &&*/ limb.owner.nutrition < NUTRITION_LEVEL_WELL_FED) 	//ORBSTATION EDIT: don't check for fatness, as we have removed that trait
		return FALSE
	return TRUE
