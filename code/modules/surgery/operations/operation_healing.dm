...
/datum/surgery_operation/basic/tend_wounds/on_success(mob/living/patient, mob/living/surgeon, tool, list/operation_args)
	...
	brute_healed += round((min(patient.get_brute_loss(), 200) * brute_multiplier), DAMAGE_PRECISION)
	burn_healed += round((min(patient.get_fire_loss(), 200) * burn_multiplier), DAMAGE_PRECISION)
