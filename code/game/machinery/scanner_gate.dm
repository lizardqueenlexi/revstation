...
/obj/machinery/scanner_gate
	...
	var/detect_nutrition = NUTRITION_LEVEL_STARVING //ORBSTATION EDIT: defeaults to starving
	...
	switch(scangate_mode)
		...
		if(SCANGATE_NUTRITION)
			if(ishuman(thing))
				...
				// if(scanned_human.nutrition >= detect_nutrition && detect_nutrition == NUTRITION_LEVEL_FAT) ORBSTATION REMOVAL: no fat check
				// 	beep = TRUE
